    ...
    for ep in range(1, cfg["epochs"]+1):
        ...
        for step, batch in enumerate(train_dl, 1):
            if batch is None:
                continue
            # ── データをデバイスに転送 ──
            batch_data = {k: v.to(args.device) for k, v in batch.items() if k not in ["audio", "texts"]}
            # ----------- flatten ------------
            ...
            physical_losses, total_physical_loss = physical_loss(model_out, train_dl,batch_data)
