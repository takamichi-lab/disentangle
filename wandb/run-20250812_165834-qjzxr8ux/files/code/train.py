...
def eval_retrieval(model, loader, device, use_wandb=True, epoch=None):
    ...
    # eval_retrieval 内：プールを削除し、全インスタンスで評価
    S_src = cosine_sim(bufs["a_src_all"], bufs["t_src_all"])   # 事前に全インスタンスを連結
    src_T2A = recall_at_k_multi(S_src,   bufs["ids_src_all"], bufs["ids_src_all"])
    src_A2T = recall_at_k_multi(S_src.T, bufs["ids_src_all"], bufs["ids_src_all"])

    S_spa = cosine_sim(bufs["a_spa_all"], bufs["t_spa_all"])
    spa_T2A = recall_at_k_multi(S_spa,   bufs["ids_spa_all"], bufs["ids_spa_all"])
    spa_A2T = recall_at_k_multi(S_spa.T, bufs["ids_spa_all"], bufs["ids_spa_all"])
