...
# ------- Leakage Index (LI) = 線形分類の精度（低いほど良い） -------
class _LinearProbe(torch.nn.Module):
    def __init__(self, in_dim: int, num_cls: int):
        super().__init__()
        self.fc = torch.nn.Linear(in_dim, num_cls)

    def forward(self, x):
        return self.fc(x)

@torch.no_grad()
def _top1_acc(logits: torch.Tensor, y: torch.Tensor) -> float:
    pred = logits.argmax(dim=1)
    return (pred == y).float().mean().item()

def leakage_probe_acc(train_x: torch.Tensor, train_y: torch.Tensor,
                      val_x: torch.Tensor,   val_y: torch.Tensor,
                      epochs: int = 5, lr: float = 1e-2, wd: float = 0.0,
                      device: str | torch.device = "cpu") -> float:
    """
    例) emb=audio_space のリーク: target=source_id
        acc が低いほど「リークが少ない」=良い
    """
    in_dim = train_x.size(1)
    num_cls = int(train_y.max().item()) + 1
    clf = _LinearProbe(in_dim, num_cls).to(device)
    ...
    clf.eval()
    with torch.no_grad():
        acc = _top1_acc(clf(val_x), val_y)
    return acc

# ------- 物理量ベースライン: 線形回帰（MAE/RMSE） -------
class _LinearReg(torch.nn.Module):
...
def linear_probe_regression(train_x: torch.Tensor, train_y: torch.Tensor,
                            val_x: torch.Tensor,   val_y: torch.Tensor,
                            epochs: int = 10, lr: float = 5e-3, wd: float = 1e-4,
                            device: str | torch.device = "cpu") -> dict:
    """
    例) 単一埋め込みベースラインに対し:
        - area[m^2], distance[m], T30[ms] を各タスクごとに回帰
    train_y, val_y: shape [N, 1] または [N, 2]（方向）
    """
    in_dim = train_x.size(1)
    out_dim = train_y.size(1)
    reg = _LinearReg(in_dim, out_dim).to(device)
    ...
    with torch.no_grad():
        pred = reg(val_x)
        mae  = (pred - val_y).abs().mean().item()
        rmse = torch.sqrt(F.mse_loss(pred, val_y)).item()
